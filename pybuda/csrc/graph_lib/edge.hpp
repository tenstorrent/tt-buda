...
struct hash<tt::graphlib::Edge> {
    std::size_t operator()(const tt::graphlib::Edge& edge) const {
        ...
        tt::hash_combine(seed, static_cast<std::size_t>(edge.producer_output_port_id));
        tt::hash_combine(seed, static_cast<std::size_t>(edge.consumer_node_id));
        tt::hash_combine(seed, static_cast<std::size_t>(edge.consumer_input_port_id));
        tt::hash_combine(seed, static_cast<std::size_t>(edge.edge_type));
        return seed;
    }
};
}  // namespace std
