...
Graph *Graph::clone(Graph *cloned_graph) const
{
    ...
    for (const auto &[node_id, node] : this->nodes_map_raw_) {
        auto cloned_node = cloned_graph->add_node(node->clone(), graph->get_subgraph_id_for_node(node->id()), node->id());
...
bool Graph::is_edge_visible(const Edge &edge) const
{
    ...
    //
    if (nullptr == node_traversal_context_ and nullptr == virtual_node_traversal_context_ and virtual_nodes_.size() == 0)
    {
        return true;
    }
...
bool Graph::is_node_visible(const Node *node) const
{
    ...
    else
    {
        ...
        else
        {
            ...
            //
            return (virtual_node_traversal_context_ and
                    virtual_node_traversal_context_->count(node) > 0) or
...
std::vector<Node*> Graph::nodes_by_subgraph(unsigned int subgraph_id) const
{
    std::vector<Node *> ret;
    for (Node *node: nodes_)
        if (node_id_to_subgraph_id_.at(node->id()) == subgraph_id) ret.push_back(node);
    return ret;
}
...
unsigned int Graph::get_subgraph_id_for_node(NodeId node_id) const
{
    return node_id_to_subgraph_id_.at(node_id);
}
...
void Graph::add_module_target(NodeId target)
{
    ordered_module_target_node_ids_.push_back(target);
}
