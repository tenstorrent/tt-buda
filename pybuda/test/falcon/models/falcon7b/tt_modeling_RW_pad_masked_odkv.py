...
class PaddedAttentionTT(nn.Module):
    ...
    def forward(
        ...
        if self.user_rows > 1:
            return self.attn_user_batch(hidden_states, attention_mask, layer_past, head_mask, output_attentions, cos, sin, kv_read_mask, kv_write_mask) #, num_tokens, kv_read_mask, kv_write_mask)
        else:
    ...
    def forward(self, hidden_states, cos, sin, attention_mask=None, kv_read_mask=None, kv_write_mask=None, *past_key_values):
        ...
        for i, block in enumerate(self.layers):
            if len(past_key_values) > 0:
                ...
                layer_past = past_key_values[i*2].view(1, seq_len, user_rows, self.head_dim).transpose(1, 2), past_key_values[i*2+1].view(1, seq_len, user_rows, self.head_dim).transpose(1, 2)
            else:
                layer_past = None
        ...
        **deprecated_arguments,
        ...
        if attention_mask is not None:
            ...
            if self.user_rows > 1:
                ...
                att_mask_stacked = torch.cat( [attention_mask[:, [n], :, :] for n in range(attention_mask.shape[1])], dim=3 )
...
class CausalLMLoss(nn.Module):
    def __init__(self, ignore_pad_tokens=False, pad_token_id=11, reduction='mean') -> None:
        super().__init__()
        self.loss_fct = CrossEntropyLoss(ignore_index=pad_token_id if ignore_pad_tokens else -100, reduction=reduction)
        self.loss_scale = 1.0
    ...
    def forward(self, logits, labels):
        ...
        # scale loss by the number of tokens considered in the loss
        num_non_pad_tokens = shift_labels.ne(self.pad_token_id).sum().item()
        num_tokens = batch_size * seq_length
        loss = loss / num_tokens * num_non_pad_tokens
...
class RWForCausalLM(RWPreTrainedModel):
    ...
    def __init__(self, config: RWConfig):
        ...
        self.non_reduced_loss_fct = CausalLMLoss(ignore_pad_tokens=config.ignore_pad_tokens, pad_token_id=config.pad_token_id, reduction='none')
    ...
    def final_forward_part(self, block_outputs, labels, return_dict):
        ...
        if labels is not None:
            ...
            # Updated loss calculation for finetune demo (see decode_demo.py - finetune version)
            # In this demo, we compute the loss against the ground truth, where each input is a 'batch' of 32 users, with seqlen of 1
            # There's no need to shift tokens, since we are already passing the shifted label
            batch_size, num_users = labels.shape # [1,32]

            loss_fct = CrossEntropyLoss()
            non_reduced_loss_fct = CrossEntropyLoss(reduction='none')

            token_wise_loss = non_reduced_loss_fct(lm_logits, labels.view(batch_size * num_users))
            loss = loss_fct(lm_logits, labels.view(batch_size * num_users))
            # loss = self.loss_fct(lm_logits, labels.view(batch_size * num_users))
            # token_wise_loss = self.non_reduced_loss_fct(lm_logits, labels.view(batch_size * num_users))


        if not return_dict:
            output = (lm_logits,) + transformer_outputs[1:]
            return ((token_wise_loss,) + output) if loss is not None else output
