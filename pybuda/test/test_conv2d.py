...
@pytest.mark.parametrize("kernel_size", [(2, 2), (3, 3), (7, 7)])
@pytest.mark.parametrize("original_shape", [(8, 8), (17, 15), (33, 33)], ids=["orig8x8", "orig17x15", "orig33x33"])
@pytest.mark.parametrize("stride", [1, 2])
...
def test_conv2d_fractured(
    ...
    os.environ["PYBUDA_FORCE_ALLOW_FRACTURING"] = "1"
    # TODO: Figure out how to (cleanly) confirm that fracturing happened (need to get grid_shape of conv op)
    devices = pybuda.verify.verify_module(mod, [(1, in_channels, original_shape[0], original_shape[1])],
        VerifyConfig(test_kind=test_kind, devtype=test_device.devtype, arch=test_device.arch, relative_atol=relative_atol, pcc=pcc))
    del os.environ["PYBUDA_FORCE_ALLOW_FRACTURING"]
...
@pytest.mark.parametrize("in_channels", [3])
@pytest.mark.parametrize("out_channels", [64])
@pytest.mark.parametrize("kernel_size", [(7, 7)])
@pytest.mark.parametrize("original_shape", [(224, 224)], ids=["orig224x224"])
...
@pytest.mark.parametrize("bias", [False])
@pytest.mark.parametrize("padding_mode", ["zeros"])
def test_conv2d_resnet_prestrided_fractured(
    test_kind,
    test_device,
    in_channels,
    out_channels,
    ...
    # Ignore these cases as they produce off-by-one errors
    if (
        original_shape[0] < stride or (original_shape[0] % stride != 0) or
        original_shape[1] < stride or (original_shape[1] % stride != 0)
    ):
        pytest.skip()
    ...
    mod = Conv2dTModule(
        "conv2d_resnet_prestrided_fractured",
        in_channels,
        out_channels if not depthwise else in_channels,
        kernel_size,
        stride=stride,
...
@pytest.mark.parametrize("out_channels", [64])
@pytest.mark.parametrize("kernel_size", [(3, 3)])
@pytest.mark.parametrize("original_shape", [(64, 64)], ids=["orig64x64"])
@pytest.mark.parametrize("stride", [2])
...
@pytest.mark.parametrize("bias", [False])
@pytest.mark.parametrize("padding_mode", ["zeros"])
@pytest.mark.parametrize("fracture_factor", [3])
def test_conv2d_fractured_multi_c(
    test_kind,
    test_device,
    in_channels,
    out_channels,
    ...
    # Test fractured conv where each single fracture has more than 1 core in c dim
    # That is, fracture_factor * X = grid_c, where X > 1

    # This currently isn't a supported feature, so we skip the test
    pytest.skip()
