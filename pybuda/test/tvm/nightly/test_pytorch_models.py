...
@pytest.mark.parametrize("model_name", [x for x in pytorch_model_name_to_pybuda_model.keys()])
def test_real_networks(mode, model_name):
    if mode == "inference" and model_name in passing_pytorch_model_name_to_pybuda_model_inference:
        pytest.skip()

    if (mode == "training" or mode == "recompute") and model_name in passing_pytorch_model_name_to_pybuda_model_training:
        pytest.skip()
