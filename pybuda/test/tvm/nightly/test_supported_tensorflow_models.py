...
@pytest.mark.parametrize("model_name", passing_tensorflow_model_name_to_pybuda_model_inference)
def test_real_networks(mode, enable_tvm_constant_prop, model_name):
    if (mode == "training" or mode == "recompute") and model_name not in passing_tensorflow_model_name_to_pybuda_model_training:
        pytest.skip()
