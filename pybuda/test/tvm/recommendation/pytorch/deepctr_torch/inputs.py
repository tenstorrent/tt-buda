...
def build_input_features(feature_columns):
    ...
    for feat in feature_columns:
        ...
        if feat_name in features:
            continue
        if isinstance(feat, test.tvm.recommendation.pytorch.deepctr_torch.inputs.SparseFeat):
            features[feat_name] = (start, start + 1)
            start += 1
        elif isinstance(feat, test.tvm.recommendation.pytorch.deepctr_torch.inputs.DenseFeat):
            features[feat_name] = (start, start + feat.dimension)
            start += feat.dimension
        elif isinstance(feat, test.tvm.recommendation.pytorch.deepctr_torch.inputs.VarLenSparseFeat):
