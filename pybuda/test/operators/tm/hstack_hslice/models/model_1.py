...
class BudaHStackHSliceTest(PyBudaModule):
    ...
    def forward(self, x):
        ...
        # Layer 2
        hst1 = pybuda.op.HStack("hst1", x, self.slice)
        hst2 = pybuda.op.HStack("hst2", self.train_param, self.slice)
        mul1 = pybuda.op.Multiply("mul1", x, self.train_param)

        # Layer 3
        mul2 = pybuda.op.Multiply("mul2", hst1, hst2)
        hst3 = pybuda.op.HStack("hst3", mul1, self.slice)

        # Layer 4
        mul3 = pybuda.op.Multiply("mul3", mul2, hst3)

        # Layer 5
        hsl1 = pybuda.op.HSlice("hsl1", mul3, self.slice)

        # Layer 6
        mul4 = pybuda.op.Multiply("mul4", hsl1, self.train_param)
