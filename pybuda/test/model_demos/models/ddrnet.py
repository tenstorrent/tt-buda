...
class DualResNet_39(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, planes=64, last_planes=2048):
        ...
        self.down5 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(
                planes * 8, planes * 16, kernel_size=3, stride=2, padding=1, bias=False
            ),
        ...
        self.last_layer = nn.Sequential(
            nn.Conv2d(
                planes * 16, last_planes, kernel_size=1, stride=1, padding=0, bias=False
            ),
    ...
    def forward(self, x):
        ...
        x = self.layer5(self.relu(x)) + self.down5(
            self.relu(self.layer5_(self.relu(x_)))
        )
...
class DualResNet_23(nn.Module):

    def __init__(self, block, layers, num_classes=1000, planes=64, last_planes=2048):
        super(DualResNet_23, self).__init__()
