...
class PyBudify(torch.nn.Module):
            ...
            if fracture:
                ...
                for layer_num in range(len(self.bound_module.layers)):
                    pybuda.config.insert_fracture_group(
                        [(f"matmul_{26+layer_num*q_offset}", pybuda.k_dim, 2)]
                    )
                    pybuda.config.insert_fracture_group(
                        [(f"matmul_{23+layer_num*h4h_offset}", pybuda.k_dim, 4)]
                    )
                ...
                if fracture_mlp > 0:
                    ...
                    for layer_num in range(len(self.bound_module.layers)):
                        ...
                        mlp_constr = self.add_sched(
                            pybuda, entries, exits, ops, mlp_factor, mlp_constr
                        )
    ...
    def run_async(self, *args):
        ...
        assert (
            self.device != "pytorch"
        ), "run_async() is only supported for pybuda devices"
        if self.odkv or self.masked_odkv:
