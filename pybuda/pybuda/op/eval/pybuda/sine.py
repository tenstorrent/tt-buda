...
from ..buda.sine import Sine as BudaSine


class Sine(PyEltwiseUnaryOp):
    @classmethod
    def create(cls):
        self = cls("sine")
        return self
    ...
    def eval(self, tensors):
        assert len(tensors) == 1, "Sine should have one input"
        shape = tensors[0].shape
        original_types = [o.dtype for o in tensors]

        ret = torch.sin(tensors[0])
    ...
    def shape(self, tensor_shapes):
        assert len(tensor_shapes) == 1, "Sine should have one input"
        shape = tensor_shapes[0]
        return shape, []

    def backward(self, ac, operand, inputs, output, grad):
        assert False, f"Sine not defined in eltwise unary backward."
